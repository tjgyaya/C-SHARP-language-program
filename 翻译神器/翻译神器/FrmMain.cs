...
namespace 翻译神器
{
    ...
    public partial class FrmMain : Form
    {
        ...
        private void button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                if (checkBox_CopyOriginalText.Checked && checkBox_CopyTranText.Checked)
                    throw new Exception("“复制翻译原文”和“复制翻译译文”不可同时启用！");

                if (!string.IsNullOrEmpty(textBox_WindowClass.Text) && !string.IsNullOrEmpty(textBox_WindowName.Text))
                    throw new Exception("请不要同时填写“窗口标题”或“窗口类名”！");
                ...
                foreach (Control ctl in groupBox_HotKey.Controls)
                {
                    ...
                    if (list.Contains(ctl.Text))
                        throw new Exception("热键存在重复！");
                    list.Add(ctl.Text);
                }
                if (list.Count == 0)
                    throw new Exception("热键为空！");
        ...
        private delegate void SetState(bool mini);
        // 设置窗口状态，隐藏或显示
        private void SetWindowState(bool mini)
        {
            if (mini && this.WindowState != FormWindowState.Minimized)
                this.WindowState = FormWindowState.Minimized;
            else if (!mini && this.WindowState != FormWindowState.Normal)
        ...
        private void ScreenTran()
        {
            ...
            Image captureImage;
            FrmScreenShot shot = new FrmScreenShot();
            SetState setState = new SetState(SetWindowState);
